...
public class MydiplomaAppTest {
	 ...
	 * "HR_MANAGER") public void EtestEditProfile() throws Exception { User u =
	 * iuser.findUserBylogin("Hr_Manager"); String hr =
	 * "{\"firstname\":\"Daryl\",\"lastname\":\"See\",\"username\":\"Hr_Manager\",\"email\":\"daryl.see@gmail.com\",\"password\":\"azerty123\",\"bio\":\"kjhfkjflkjflkjfljljojpoj\" ,\"valid\":\"true\"}"
	 * ; MockMultipartFile image = new MockMultipartFile("file", "07.PNG", "",
	 * "".getBytes(StandardCharsets.UTF_8));
	 * System.out.println("*****"+image.getName()); ObjectMapper objectMapper = new
	 * ObjectMapper(); User result = objectMapper.readValue(String.valueOf(hr),
	 * User.class);
	 * 
	 ...
	 * objectMapper.writeValueAsString(result).getBytes(StandardCharsets.UTF_8));
	 * 
	 * mockMvc.perform(multipart("/api/user/edit-profile/" +
	 * u.getId()).file(image).file(metadata)
	...
	@Test
	@WithMockUser(username = "Hr_Manager", password = "azerty123", authorities = "HR_MANAGER")
	public void NtestInterview() throws Exception {
		String c = "{    \"interviewDate\":\"2022-01-20\",\r\n" + "   \"interviewTime\":\"18:00:00\",\r\n"
				+ "   \"endDate\":\"2022-01-20\",\r\n" + "   \"endTime\":\"19:00:00\"   \r\n" + "         \r\n" + "}";
		User u = iuser.findUserBylogin("Intern_Intern");
		Vacancy v = ivacancy.getVacancyByTitle("test");

		Application app = iapp.getApplicationByInternAndVacancy(v, u);

		MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders
				.post("/api/interview/plannify/" + app.getId()).content(c).accept(MediaType.APPLICATION_JSON)
				.contentType(MediaType.APPLICATION_JSON);
	...
	public void testAUploadTaskDemo() throws Exception {
		String demo = "{\"category\":\"PDF\",\"description\":\"jnkjkjk\"}";
		Task t = itaskRepo.findByTaskName("task1");
		User intern = iuser.findUserBylogin("Intern_Intern");
		MockMultipartFile file = new MockMultipartFile("file", "Rapport.pdf", MediaType.APPLICATION_PDF_VALUE,
				"".getBytes(StandardCharsets.UTF_8));
